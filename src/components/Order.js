...
const Order = () => {
  ...
  const handlePayment = async () => {
    if (!stripe || !elements) {
      setError(
        "Stripe.js hasn't loaded yet. Please wait a moment and try again."
      );
      return;
    }
    ...
    if (
      !cardElement ||
      !cardElement._frame ||
      !cardElement._frame.contentWindow
    ) {
      setError("Invalid card element. Please try again.");
      return;
    }

    const cardNumber =
      cardElement._frame.contentWindow.document.querySelector(
        "[name=cardnumber]"
      );
    ...
    try {
      ...
      toast.success("Payment received successfully!", {
        onClose: () => {
          navigate("/"); // Navigate back to the home page after the toast is closed
        },
      });
    } catch (error) {
